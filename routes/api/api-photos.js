...
//----------------------------------------------------------------------------------
// List all photos
// Return JSON string of all photos
...
router.get('/:photoid', async (req, res, next)=>{ //handle get request from URL with path /photos/photoid
  try {
  console.log("api-photos.js LINE 49   finding "+req.params.photoid);
  console.log("\n photos.js LINE 50: req.params = ", req.params, "\n");
...
router.put('/:photoid', async (req, res, next) => {
  try {
    ...
    console.log('api-photos.js  LINE 74:   req.body = ', req.body);

    console.log('api-photos.js  LINE 76:   data = ', data);
...
router.put('/addComment/:photoid', async (req, res, next) => {
  try {
...
router.delete('/delete/:photoid', async (req, res, next)=>{// Delete a picture 
  ...
  try {
    ...
    res.send(JSON.stringify(photoToDelete));
  } 
  catch (err) {
...
router.post('/', upload.single('image'), async (req, res, next) => {
  try {
    ...
    const newPhoto = await PhotoService.create(photoData); // Save the photo to the database
